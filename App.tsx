...
import { UploadCloud, Image as ImageIcon, Download, Sparkles, RefreshCcw, Globe, Coins, User as UserIcon, LogOut, Moon, Sun, HelpCircle, Mail, Layers, Zap, Settings2, ShieldCheck, FileText, Info } from 'lucide-react';
import { ProductCategory, SceneType, LightingType, ProcessingState, Language, User, Currency, ProcessingProvider } from './types';
...
const App: React.FC = () => {
  ...
  const [engineProvider, setEngineProvider] = useState<ProcessingProvider>('n8n');
  ...
  const handleProcessImage = async () => {
    ...
    setProcessingState({ 
      ...
      step: 'analyzing', 
      error: null,
      batchProgress: { current: 0, total: cost }
    });
    ...
    try {
      ...
      for (let i = 0; i < selectedScenes.length; i++) {
        setProcessingState(prev => ({
          ...prev, 
          step: 'generating',
          batchProgress: { current: i + 1, total: cost }
        }));
        ...
        if (engineProvider === 'n8n') {
          resultUrl = await processWithN8n(rawBase64, selectedCategory, currentScene, selectedLighting);
        } else {
          resultUrl = await processWithGemini(rawBase64, mimeType, selectedCategory, currentScene, selectedLighting);
        }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-apple-bg dark:bg-anthracite-900 font-sans transition-colors duration-500">
      ...
      <ImageViewer 
        isOpen={isViewerOpen} 
        imageUrl={processedImages[viewerIndex]} 
        category={selectedCategory}
        scene={selectedScenes[viewerIndex] || selectedScenes[0]}
          ...
          <div className="flex items-center gap-1 md:gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={currencyDropdownRef}>
                <button onClick={() => setIsCurrencyOpen(!isCurrencyOpen)} className="p-2 text-gray-500 hover:text-black dark:text-gray-400 dark:hover:text-white transition-colors flex items-center gap-1 uppercase text-[10px] font-black"><Coins className="w-4 h-4" /> {currency}</button>
                {isCurrencyOpen && (
                  <div className="absolute top-full mt-2 right-0 bg-white dark:bg-anthracite-800 border border-gray-200 dark:border-white/5 rounded-xl shadow-2xl p-1 w-24 overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
                    {['TRY', 'USD', 'EUR'].map(c => (
                      <button key={c} onClick={() => { setCurrency(c as Currency); setIsCurrencyOpen(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold ${currency === c ? 'bg-black text-white dark:bg-white dark:text-black' : 'hover:bg-gray-50 dark:hover:bg-anthracite-700 text-gray-700 dark:text-gray-300'}`}>{c}</button>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-2 ml-2">
              {user ? (
                <div className="flex items-center gap-3 bg-gray-50 dark:bg-anthracite-800/50 border border-gray-100 dark:border-white/10 rounded-full pl-3 pr-1 py-1 shadow-sm transition-colors">
                  ...
                  <button onClick={() => setShowPricing(true)} className="bg-black dark:bg-white text-white dark:text-black text-[10px] font-black px-3 py-1.5 rounded-full hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors uppercase">{t.buyCredits}</button>
                  <button onClick={handleLogout} className="p-1.5 hover:bg-gray-100 dark:hover:bg-anthracite-700 rounded-full text-gray-400 hover:text-red-500 transition-colors"><LogOut className="w-3.5 h-3.5" /></button>
                </div>
              ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
                ...
                <div className="flex flex-col gap-6 w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
                    ...
                    {originalImage && processedImages.length === 0 && (
                        ...
                        disabled={processingState.isProcessing} 
                        className={`w-full py-5 rounded-[2rem] font-black text-lg shadow-2xl flex items-center justify-center gap-3 transition-all duration-500 uppercase tracking-widest ${processingState.isProcessing ? 'bg-gray-100 dark:bg-anthracite-700 text-gray-400 cursor-wait' : 'bg-black dark:bg-white text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 hover:shadow-black/10 dark:hover:shadow-white/10 hover:scale-[1.01] active:scale-[0.98]'}`}
                        >
                        {processingState.isProcessing ? (<><RefreshCcw className="w-6 h-6 animate-spin" />{t.processing}</>) : (<><Sparkles className="w-6 h-6" />{isBatchMode ? t.batchProcessButton : t.processButton} <span className="bg-white/20 dark:bg-black/10 px-3 py-1 rounded-full text-[12px] ml-2 font-black tracking-tighter">{selectedScenes.length} {t.credits}</span></>)}
                        </button>
                    )}
                </div>

                {/* Result Side */}
                <div className={`relative rounded-[2.5rem] h-[550px] flex flex-col items-center justify-center bg-white dark:bg-anthracite-800 border border-gray-100 dark:border-white/5 overflow-hidden transition-all duration-700 ${processedImages.length > 0 ? 'shadow-2xl ring-1 ring-black/5 dark:ring-white/5' : 'shadow-sm'} animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-150`}>
                    <ProcessingOverlay isProcessing={processingState.isProcessing} lang={lang} batchProgress={processingState.batchProgress} />
                    {processedImages.length > 0 ? (
                        <div className="w-full h-full relative bg-gradient-to-br from-gray-50/50 to-white/50 dark:from-anthracite-900/50 dark:to-anthracite-800/50 overflow-y-auto custom-scrollbar p-10">
                            <div className={`grid gap-8 ${processedImages.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                                {processedImages.map((img, idx) => (
                                <div key={idx} className="relative aspect-square group cursor-zoom-in animate-in zoom-in slide-in-from-bottom-4 duration-700 delay-75" onClick={() => openViewer(idx)}>
                                    <img src={img} alt={`Result ${idx}`} className="w-full h-full object-contain drop-shadow-2xl rounded-3xl bg-white dark:bg-anthracite-900 p-6 border border-gray-100 dark:border-white/5 transition-all group-hover:scale-[1.02] group-hover:shadow-2xl" />
                                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all rounded-3xl flex items-center justify-center opacity-0 group-hover:opacity-100 backdrop-blur-[1px]">
                                        <div className="bg-white text-black px-5 py-2.5 rounded-full font-black text-[10px] uppercase tracking-widest shadow-2xl transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                                            İncele & İndir
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="absolute top-8 left-8 bg-black/90 dark:bg-white text-white dark:text-black px-5 py-1.5 rounded-full text-[10px] font-black shadow-2xl uppercase tracking-widest z-20 border border-white/10 dark:border-black/5 flex items-center gap-2">
